...
function AddCourseModal({close}) {
  ...
  if (courseName != '' && courseId != ''  && courseProfessor != '' && courseType != '' ){
    const { data, error } = await supabase
      .from('Courses')
      .insert([{ course_id: courseId, course_professor: courseProfessor, course_name: courseName, course_type: courseType, course_time: coursetime, discord_link: discordLink, course_location: courseLocation }])
      if (error){
        toast.error(error, {
          position: "top-center",
          autoClose: 2500,
