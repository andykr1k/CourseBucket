  ...
  return (
    <div className="">
      ...
      <div className='absolute z-10 w-full h-modal pointer-events-none'>
      {
        ...
        <div className='pointer-events-auto'>
          <AddCourseModal/>
          <button className='fixed bottom-0 left-0 m-3' onClick={() => setModal(false)}><AiOutlineCloseCircle size={36}/></button>
        </div>
        :
        <div className=''></div>
      }
    ...
    <form className='mx-10 md:mx-48 p-10 pb-8 '>
      ...
      supadata && supadata.length>0 && supadata.map((item)=>
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="w-full h-42 p-6 bg-white border border-gray-200 rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700">
          <div>
            <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{item.course_id}</h5>
            <h5 className="mb-3 font-normal text-gray-700 dark:text-gray-400 whitespace-nowrap overflow-x-auto scrollbar-hide">{item.course_name}</h5>
          </div>
          <div className=''>
      ...
      :
        sortedCourses && sortedCourses.length>0 && sortedCourses.map((item)=>
          <div className="w-full h-42 p-6 bg-white border border-gray-200 rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700">
